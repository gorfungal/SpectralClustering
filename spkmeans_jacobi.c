...
double** matrix_allocation(n, k)
{
    ...
    for (int point_i = 0; point_i < n; point_i++) {
        ...
        if (!mat[point_i]) printf("problem with memory allocation mat");
...
void p_rot_mat(double** a_mat, double** v_mat, double* a_off_diag_sum, int* current_iter, int n) {
    ...
    for (i = 0; i < n; i++) { 
        ...
        new_a_mat[i][i_max] = temp_a_value;
        new_a_mat[i_max][i] = temp_a_value;
    }
    //column j
    for (i = 0; i < n; i++) {
        temp_a_value = c * a_mat[i][j_max] + s * a_mat[i][i_max];
        new_a_mat[i][j_max] = temp_a_value;
        new_a_mat[j_max][i] = temp_a_value;
    }
    //a_ij and a_ji
    new_a_mat[i_max][j_max] = 0;
    new_a_mat[j_max][i_max] = 0;
    //a_ii
    temp_a_value = c * c * a_mat[i_max][i_max] + s * s * a_mat[j_max][j_max] - 2 * s * c * a_mat[i_max][j_max];
    new_a_mat[i_max][i_max] = temp_a_value;
    //a_jj
    temp_a_value =  s * s * a_mat[i_max][i_max] + c * c * a_mat[j_max][j_max] + 2 * s * c * a_mat[i_max][j_max];
    new_a_mat[j_max][j_max] = temp_a_value;
    //sum squere all matrix
    for (i = 0; i < n; i++) {
        for (j = i+1; j < n; j++) {
            new_a_off_diag_sum += 2 * pow(new_a_mat[i][j], 2.0);
        }
    }
    printf("new_a_mat\n");
...
double* return_list(char* goal, double** mat_a, double** mat_b, int n, int k) {
    double* rl;
    int i = 0, j = 0;
    if (strcmp(goal, "jacobi") == 0) {
        ...
        rl = calloc(n * n + 2, sizeof(double));
        if (!rl) printf("problem with memory allocation mat");
        rl[0] = (double)k;
        rl[1] = (double)n;
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                rl[i * n + j + 2] = mat_a[i][j];
            }
        }
        return rl;
    }
...
double* spkmeans(char* goal, char* filename, int* mat_size, int k) {
    ...
    if (strcmp(goal, "wam") == 0) {
        ...
        return return_list(goal, w_mat, w_mat, n, n);
    }
    double** diag = diagonal_mat(n, w_mat);
    if (strcmp(goal, "ddg") == 0) {
        free_matrix_allocation(points, n);
        free_matrix_allocation(w_mat, n);
        return return_list(goal, diag, diag, n, n);
    }
    ...
    if (strcmp(goal, "lnorm") == 0) {
        ...
        return return_list(goal, lnorm, lnorm, n, n);
    }
    // L_Norm -> A (Eigenvalues of L_Norm)
    double** v_mat = jacobi(lnorm, n);
    if (strcmp(goal, "jacobi") == 0) {
        free_matrix_allocation(points, n);
        free_matrix_allocation(w_mat, n);
        free_matrix_allocation(diag, n);
        return return_list(goal, v_mat, v_mat, n, n);
    }
}
